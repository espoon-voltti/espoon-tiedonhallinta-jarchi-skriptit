...
import com.archimatetool.editor.browser.BrowserEditor;
...
public class HTMLReportExporter {
    ...
    public void preview() throws Exception {
        ...
        BrowserEditorInput input = new BrowserEditorInput("file:///" + file[0].getPath(), Messages.HTMLReportExporter_0 + " " + fModel.getName()); //$NON-NLS-1$
    ...
    public File createReport(File targetFolder, String indexFileName, IProgressMonitor monitor) throws IOException {
        ...
        File mainFile = new File(EspooHtmlReportsPlugin.INSTANCE.getTemplatesFolder(), "st/main.stg"); //$NON-NLS-1$
        ...
        stModel.add("strategyFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.STRATEGY))); //$NON-NLS-1$
        stModel.add("businessFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.BUSINESS))); //$NON-NLS-1$
        stModel.add("applicationFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.APPLICATION))); //$NON-NLS-1$
        stModel.add("technologyFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.TECHNOLOGY))); //$NON-NLS-1$
        stModel.add("motivationFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.MOTIVATION))); //$NON-NLS-1$
        stModel.add("implementationFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.IMPLEMENTATION_MIGRATION))); //$NON-NLS-1$
        stModel.add("otherFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.OTHER))); //$NON-NLS-1$
        stModel.add("relationsFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.RELATIONS))); //$NON-NLS-1$
        stModel.add("viewsFolder", (IFolder)EspooElementInvocationHandler.wrapEspooProxy(fModel.getFolder(FolderType.DIAGRAMS))); //$NON-NLS-1$
    ...
    private void copyHTMLSkeleton(File targetFolder) throws IOException {
        ...
        File srcDir = new File(EspooHtmlReportsPlugin.INSTANCE.getTemplatesFolder(), "html"); //$NON-NLS-1$
    ...
    private void writeElement(File elementFile, ST stFrame, EObject component) throws IOException {
        ...
        stFrame.add("element", wrappedComponent); //$NON-NLS-1$
    ...
    private void writeDiagrams(File imagesFolder, File viewsFolder, ST stFrame) throws IOException {
        ...
        for(IDiagramModel dm : diagramModels) {
            ...
            IArchimateElement isviewfor = null;
            String kuvattavaKohde = null;
            for(IProperty prop: dm.getProperties()) {
            	if ("_KuvattavaKohde".equals(prop.getKey())) {
            		kuvattavaKohde = prop.getValue();
            		break;
            	}
            }
            ...
            for(IDiagramModelObject dmo: dm.getChildren() ) {
                ...
                if (kuvattavaKohde != null && dmo instanceof IDiagramModelArchimateObject) {
                	IArchimateElement ae = ((IDiagramModelArchimateObject)dmo).getArchimateElement();
                	if (ae != null && kuvattavaKohde.equals(ae.getId())) {
                		isviewfor = (IArchimateElement)EspooElementInvocationHandler.wrapEspooProxy(ae);
                	}
                }
            }
            ...
            stFrame.remove("element"); //$NON-NLS-1$
            stFrame.remove("isviewfor");
            stFrame.add("isviewfor", isviewfor);
            EObject wrappedDm = dm;
            if (dm instanceof IProperties && !(dm instanceof IEspooElementViewReferences)) {
            	wrappedDm = (EObject)EspooElementInvocationHandler.wrapEspooProxy(dm);
            }
            stFrame.add("element", wrappedDm); //$NON-NLS-1$
